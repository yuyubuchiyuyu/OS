...
public class UnaryExp {
    ...
    public Integer calculate(LLVMSymbolTable LLVMSymbolTable) {
        if (this.primaryExp != null) {
            return this.primaryExp.calculate(LLVMSymbolTable);
            ...
            if (character == Token.tokenType.PLUS) {
                return this.unaryExp.calculate(LLVMSymbolTable);
            } else if (character == Token.tokenType.MINU) {
                return -this.unaryExp.calculate(LLVMSymbolTable);
            } else if (character == Token.tokenType.NOT) {  // !a
                if (this.unaryExp.calculate(LLVMSymbolTable) != 0) {
                    return 0;
                } else {
                    return 1;
                }
    ...
    public boolean judgeCalculate(LLVMSymbolTable LLVMSymbolTable) {
        if (this.primaryExp != null) {
            return this.primaryExp.judgeCalculate(LLVMSymbolTable);
        } else if (this.ident != null) {
            return false;
        } else if (this.unaryOp != null) {
            return this.unaryExp.judgeCalculate(LLVMSymbolTable);
        }
        return true;
    }
    ...
    public ArrayList<Value> getCalInstructions(LLVMSymbolTable LLVMSymbolTable) {
        ...
        if (this.primaryExp != null) {
            return this.primaryExp.getCalInstructions(LLVMSymbolTable);
        } else if (this.ident != null) {
            LLVMSymbol LLVMSymbol = LLVMSymbolTable.search(ident);
            String funcName = LLVMSymbol.irName;
            IrType funcIrType = LLVMSymbol.irType;
            String resName = "%LocalVariable_" + Count.getFuncInner();
            CallInst callInst = new CallInst(resName, funcIrType, funcName, funcRParams, LLVMSymbolTable);
            FunctionThing.isCalled.add(funcName);
    ...
    public IrType getIrType(LLVMSymbolTable LLVMSymbolTable) {
        if (this.primaryExp != null) {
            return this.primaryExp.getIrType(LLVMSymbolTable);
        } else if (this.ident != null) {
            return LLVMSymbolTable.search(ident).irType;
        } else {
            return unaryExp.getIrType(LLVMSymbolTable);
        }
    }

    public LLVMSymbol judgeSymbolPure(LLVMSymbolTable LLVMSymbolTable) {
        if (this.primaryExp != null) {
            return this.primaryExp.judgeSymbolPure(LLVMSymbolTable);
        } else if (this.ident != null) {
