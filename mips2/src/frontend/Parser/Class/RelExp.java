...
public class RelExp {
    ...
    public boolean judgeGetTF(LLVMSymbolTable LLVMSymbolTable) {   // 是否直接能得到结果
        if (!addExp.judgeCalculate(LLVMSymbolTable)) {
            return false;
        }
        ...
        while (relTemp != null) {
            ...
            if (!addTemp.judgeCalculate(LLVMSymbolTable)) {
                return false;
            }
    ...
    public Integer getTFResult(LLVMSymbolTable LLVMSymbolTable) {   // 得到结果
        Integer res = addExp.calculate(LLVMSymbolTable);

        RelExpTemp relTemp = this.relExpTemp;
        while (relTemp != null) {
            Token.tokenType type = relTemp.getCharacterType();
            AddExp addTemp = relTemp.getAddTemp();
            Integer resTemp = addTemp.calculate(LLVMSymbolTable);
    ...
    public ArrayList<Value> getInstructions(LLVMSymbolTable LLVMSymbolTable) {
        ...
        String resName = null;
        if (addExp.judgeCalculate(LLVMSymbolTable)) {
            res1 = addExp.calculate(LLVMSymbolTable);
        } else {
            instructions.addAll(addExp.getCalInstructions(LLVMSymbolTable));
        ...
        while (relTemp != null) {
            ...
            AddExp addTemp = relTemp.getAddTemp();
            if (addTemp.judgeCalculate(LLVMSymbolTable)) {
                res2 = addTemp.calculate(LLVMSymbolTable);
            } else {
                instructions.addAll(addTemp.getCalInstructions(LLVMSymbolTable));
                TransferInst.typeTransfer(instructions,irType);
