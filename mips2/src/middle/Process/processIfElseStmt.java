...
public class processIfElseStmt {
    public static ArrayList<Value> processIfElseStmt(Stmt stmt, LLVMSymbolTable LLVMSymbolTable, String funcName, String continueName, String breakName) {
        // 没有 进入 cond 的标签
        ...
        if (elseStmt != null) {
            instructions.addAll(processCond(cond, LLVMSymbolTable, ifLabel, elseLabel));
            // if
            LabelInst ifLabelInst = new LabelInst(ifLabel);
            instructions.add(ifLabelInst);
            instructions.addAll(processStmt.processStmt(ifStmt, LLVMSymbolTable, funcName, continueName, breakName));
            ...
            instructions.addAll(processStmt.processStmt(elseStmt, LLVMSymbolTable, funcName, continueName, breakName));
            ...
            instructions.addAll(processCond(cond, LLVMSymbolTable, ifLabel, endLabel));
            // if
            LabelInst ifLabelInst = new LabelInst(ifLabel);
            instructions.add(ifLabelInst);
            instructions.addAll(processStmt.processStmt(ifStmt, LLVMSymbolTable, funcName, continueName, breakName));
    ...
    public static ArrayList<Value> processCond(Cond cond, LLVMSymbolTable LLVMSymbolTable, String ifLabel, String elseLabel) {
        ArrayList<Value> instructions = new ArrayList<>();
        ...
        for (int i = 0; i < lAndExps.size(); i++) {
            ...
            for (int j = 0; j < lAndExp.eqExps.size(); j++) {
                ...
                if (eqExp.judgeGetTF(LLVMSymbolTable)) {
                    Integer res = eqExp.getTFResult(LLVMSymbolTable);
                    ...
                    instructions.addAll(eqExp.getInstructions(LLVMSymbolTable));
                    if (!Optimize.optimize3) {
